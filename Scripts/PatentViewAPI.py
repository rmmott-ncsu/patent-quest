...
# of records pulled per call. Max = 1000
STEP_SIZE = 1000
...
# API was responding with duplicates of each record. one didn't contain cpc data. this fixed it. but then they fixed it.
# DONT USE
# Create json file and iteratively call patents API
def get_all_patents_duplicates(start_id: int, end_id: int, outfile: str):
    # Create or overwrite existing file and set up patents object for valid json
    with open(outfile, "w") as file:
        file.write("{\n    \"patents\": [\n")
        file.close()
...
def main():
    ...
    # get_all_patents(8341762, 8341770, 'patents_test.json')
